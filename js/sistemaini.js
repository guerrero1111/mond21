...
jQuery("body").on('hide.bs.modal','#modalMessage_preg',function(e){	
	...
	window.location.href = '/cerremos18/'+$catalogo;						    
	//window.location.href = '/holanda/registrar_facebook/0';
});	
			...
			success: function(data){
				...
				if(data.exito != true){
					...
					jQuery('#msg_general').html(data.general);
						...
						spinner.stop();
						jQuery('#foo').css('display','none');
						window.location.href = '/cerremos18/'+data.redireccion;    //$catalogo;						

						/*
						//new ok 
						var url = "/cerremos18/proc_modal_instrucciones";
						//alert(url);
					...
					jQuery('#msg_contrasena').html(data.contrasena);
  				    jQuery('#msg_general').html(data.general);
						...
						if (data.jugo!= true) {
							...
							window.location.href = '/cerremos18/';    
						}
						*/
				}
			} 
...
//         var url = "/cerremos18/proc_modal_pregunta";
...
// }	
        ...
        jQuery('body').on('click','.card[valor="n"]', function (e) {               
          ...
          jQuery.ajax({ //guardar en la cookie el conteo
                    url : '/cerremos18/respuesta_tarjeta',
                    data : { 
                        ...
                        tiempo   :  $('span.r1').text(),  //tiempo en que se viro esta tarjeta 
                    ...
                    success : function(data) {  
                        ...
                        if (data.redireccion==0) {
										localStorage.clear();
										localStorage.setItem('modal_preg', true);
		                                var url = "/cerremos18/proc_modal_facebook";
		                                jQuery('#modalMessage_preg').modal({
		                                      show:'true',
		                                    remote:url,
		                                }); 

                        }  else if (data.redireccion!=88) {
									clearInterval(interval);  //limpiar el intervalo
							        localStorage.clear(); //quitar las variables del localStorage													    		
							    		//comenzar un nuevo juego 
									var url = "/cerremos18/proc_modal_reintentar";
        ...
        //jQuery( ".card" ).one( "click", function() {
        jQuery("body").one('click','.tablero',function(e){  
        //jQuery("body").on('click','#btn_tiempo',function(e){  
...
///////////////////////////////////////////////////////////
  ...
  //este btn aparece en la ventana de respuesta modal_instrucciones
       jQuery('body').on('click','.btn_respuesta2', function (e) {  
              e.preventDefault();
                jQuery.ajax({ //guardar en la cookie el conteo
                        url : '/cerremos18/respuesta_juego',
                        data : { 
